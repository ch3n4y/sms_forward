...
-- 通知类型, 支持配置多个
-- NOTIFY_TYPE = { "custom_post", "telegram", "pushdeer", "bark", "dingtalk", "feishu", "wecom", "pushover", "inotify", "next-smtp-proxy", "gotify", "serverchan", "usb_uart" }
NOTIFY_TYPE = { "usb_uart" }

-- usb_uart 通知配置, 通过 UART3 输出 JSON Lines
-- 推荐搭配: NOTIFY_TYPE = { "bark", "usb_uart" }
...
-- BARK_API = "https://bark.256404.xyz/aahevpMRyEM8J42Eooh4BJ"
-- BARK_KEY = "aahevpMRyEM8J42Eooh4BJ"
...
-- 开机通知
BOOT_NOTIFY = true
-- 开机通知前缀: "phone_imsi"=本机号末位→IMSI末位; "imsi_tail"=仅 IMSI; 其它字符串=固定前缀(""=不要前缀)
BOOT_NOTIFY_PREFIX = "phone_imsi"
-- 本机号与 IMSI 均取末尾若干位(仅数字); 可在 config 中配置 NUMBER = "13800138000" 作为读不到 CNUM 时的号码
BOOT_NOTIFY_PREFIX_TAIL_LEN = 4
