...
local function smsCallback(sender_number, sms_content, datetime)
    ...
    -- 判断音量
    if config.AUDIO_VOLUME == 0 or config.AUDIO_VOLUME == nil then
        return
    end
    ...
    -- 判断 SMS_TTS 开关
    if type(config.SMS_TTS) ~= "number" or config.SMS_TTS == 0 then
        return
    end

    -- TTS 仅播报验证码
    if config.SMS_TTS >= 1 then
