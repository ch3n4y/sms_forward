local function tts(text, vol)
    ...
    vol = vol or config.AUDIO_VOLUME or 1
...
local options = {
    {
        name = function()
            return "扬声器音量 " .. (config.AUDIO_VOLUME or 0)
        end,
        func = function()
            local vol = config.AUDIO_VOLUME or 0
            vol = vol >= 6 and 0 or vol + 1
            config.AUDIO_VOLUME = vol
            tts("音量 " .. vol)
        end,
    },
    {
        name = function()
            return "通话音量 " .. (config.CALL_VOLUME or 0)
        end,
        func = function()
            local vol = config.CALL_VOLUME or 0
            vol = vol >= 7 and 0 or vol + 1
            vol = vol == 1 and 3 or vol
            vol = vol == 2 and 3 or vol
            config.CALL_VOLUME = vol
            tts("音量 " .. vol)
        end,
    },
    {
        name = function()
            return "麦克音量 " .. (config.MIC_VOLUME or 0)
        end,
        func = function()
            local vol = config.MIC_VOLUME or 0
            vol = vol >= 7 and 0 or vol + 1
            vol = vol == 1 and 3 or vol
            vol = vol == 2 and 3 or vol
    ...
    },
    {
        ...
        func = function()
            ...
            local currentOptionIndex = config.CALL_IN_ACTION or 0
            currentOptionIndex = currentOptionIndex >= #options - 1 and 0 or currentOptionIndex + 1
            config.CALL_IN_ACTION = currentOptionIndex
        ...
        func = function()
            if type(CALL_NUMBER) == "string" and CALL_NUMBER ~= "" then
                ...
                sys.taskInit(function()
                    local old_call_in_action = config.CALL_IN_ACTION
                    log.info("handler_powerkey", "临时修改来电动作配置项", 0)
                    config.CALL_IN_ACTION = 0
                    sys.waitUntil("CALL_DISCONNECTED", 1000 * 60 * 2)
                    config.CALL_IN_ACTION = old_call_in_action
    ...
    },
    {
        ...
        func = function()
            ...
            local currentOptionIndex = config.SMS_TTS or 0
            currentOptionIndex = currentOptionIndex >= #options - 1 and 0 or currentOptionIndex + 1
            config.SMS_TTS = currentOptionIndex
            tts(options[currentOptionIndex + 1])
        end,
    },
    {
    ...
    },
    {
        name = "开机通知",
        func = function()
            config.BOOT_NOTIFY = not config.BOOT_NOTIFY
            tts("开机通知 " .. (config.BOOT_NOTIFY and "开" or "关"))
        end,
    },
    {
        name = "查询流量",
    ...
    },
    {
        name = "网卡",
        func = function()
            config.RNDIS_ENABLE = not config.RNDIS_ENABLE
            if config.RNDIS_ENABLE then
                ril.request("AT+RNDISCALL=1,0")
            else
                ril.request("AT+RNDISCALL=0,0")
            end
            tts("网卡 " .. (config.RNDIS_ENABLE and "开" or "关"))
        end,
    },
    {
        name = "状态指示灯",
        func = function()
            local state = not config.LED_ENABLE
            pmd.ldoset(state and 1 or 0, pmd.LDO_VLCD)
            config.LED_ENABLE = state
...
local function longCb()
    if cc.anyCallExist() then
        ...
        if cc_state == cc.INCOMING or cc_state == cc.WAITING or cc_state == cc.HOLD then
            ...
            if config.CALL_IN_ACTION == 4 then
                sys.taskInit(function()
                    log.info("handler_powerkey", "临时修改来电动作配置项", 0)
                    config.CALL_IN_ACTION = 0
                    sys.waitUntil("CALL_DISCONNECTED", 1000 * 60 * 2)
                    config.CALL_IN_ACTION = 4
...
powerKey.setup(nil, nil, restartCb)
