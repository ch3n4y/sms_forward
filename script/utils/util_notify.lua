...
-- 消息队列
local msg_queue = {}
local usb_uart_ready = false
local USB_UART_CHANNEL = "usb_uart"
local USB_NOTIFY_UART_ID = 3

local function getUsbNotifyEol()
    if type(config.USB_NOTIFY_EOL) == "string" and config.USB_NOTIFY_EOL ~= "" then
        return config.USB_NOTIFY_EOL
    end
    return "\r\n"
end

local function formatNotifyTimestamp()
    return util_status.formatTimestamp()
end

local function initUsbNotifyUart()
    if usb_uart_ready then
        return true
    end
    if config.USB_NOTIFY_ENABLE == false then
        log.error("util_notify", "已禁用 `config.USB_NOTIFY_ENABLE`")
        return false
    end
...
local function isLocalChannel(channel)
    return channel == USB_UART_CHANNEL
end

local function canSendNow(channel)
    if isLocalChannel(channel) then
        return true
    end
    return net.getState() == "REGISTERED"
end

local function popNextReadyItem()
    for index, item in ipairs(msg_queue) do
        if canSendNow(item.channel) then
            table.remove(msg_queue, index)
            return item
        end
    end
end
...
local notify = {
    ...
    -- 发送到 usb_uart
    [USB_UART_CHANNEL] = function(msg)
        if not initUsbNotifyUart() then
            return 500, nil, "usb uart init failed"
        end

        local body = {
            channel = USB_UART_CHANNEL,
            message = msg,
        ...
        if not ok then
            ...
            return 500, nil, "usb uart write failed"
        end
...
function send(msg, channel)
    ...
    if type(channel) ~= "string" or channel == "" then
        log.error("util_notify.send", "发送通知失败", "通知渠道为空", tostring(channel))
        return true
    end
    if notify[channel] == nil then
